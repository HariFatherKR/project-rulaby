...
interface RuleLeaderboardEntry {
  ruleId: string;
  title: string;
  category: string;
  usageCount: number;
...
export async function handleAnalyticsTool(name: string, args: any) {
  ...
  switch (name) {
    ...
    case 'get_rule_leaderboard': {
      ...
      const usageData: Record<string, { count: number; recent: number }> = {};
      ...
      for (const file of usageFiles) {
        try {
          ...
          const usage: RuleUsageEntry = JSON.parse(content);
    ...
    case 'get_usage_analytics': {
      ...
      const analytics: RuleUsageEntry[] = [];
...
async function updateLeaderboard(ruleId: string) {
