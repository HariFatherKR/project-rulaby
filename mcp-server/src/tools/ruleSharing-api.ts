...
export async function handleShareRules(request: any) {
  try {
    const args = request.params.arguments || {};
    const {
      includeProjectSpecific = true,
      expiresInDays = 1, // Default to 24 hours
      maxUses = null,
    } = args;
    ...
    if (detectedRules.length === 0) {
      throw new McpError(
        ...
        "No rule files found in the current directory. Please ensure you have IDE rule files (e.g., .cursorrules, .windsurfrules, etc.)"
      );
    }
    ...
    const encryptionResult = await encryptor.encrypt(
      JSON.stringify(urf),
      password
    );
      ...
      content: [
        {
          type: "text",
          text:
            `✅ Your ${currentIDE} rules have been shared successfully!\n\n` +
            `📋 **Share Code**: \`${shareResponse.shareCode}\`\n` +
            `🔐 **Password**: \`${password}\`\n\n` +
            `📁 **Files shared**: ${detectedRules
              .map((r) => r.filePath)
              .join(", ")}\n` +
            `📊 **Total size**: ${(urf.metadata.totalSize / 1024).toFixed(
              2
            )} KB\n` +
            `⏰ **Expires**: ${new Date(
    ...
    throw new McpError(
      ...
      `Failed to share rules: ${error.message}`
    );
  }
}

export async function handleImportRules(request: any) {
  try {
    const args = request.params.arguments || {};
    ...
    if (!shareCode || !password) {
      throw new McpError(
        ...
        "Both shareCode and password are required"
      );
    }
    ...
    if (!shareCodeGen.validate(shareCode)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        "Invalid share code format. Expected: RULABY-XXXX-XXXX"
      );
    }
    ...
    if (detectedIDE === "unknown" && !targetIDE) {
      throw new McpError(
        ErrorCode.InvalidRequest,
        "Could not detect your IDE. Please specify targetIDE parameter."
      );
    }
    ...
    throw new McpError(
      ErrorCode.InternalError,
      `Failed to import rules: ${error.message}`
    );
  }
}
