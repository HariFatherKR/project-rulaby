    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to read session: ${error}`);
            }
        });
        // Handle tool calls
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            switch (request.params.name) {
                case 'create_memory_session': {
                    const { title, contextProfile, promptRules } = request.params.arguments;
                ...
                case 'add_memory_entry': {
                    const { sessionId, role, content } = request.params.arguments;
                    const filePath = path.join(DATA_DIR, `${sessionId}.json`);
                    try {
                        ...
                        throw new McpError(ErrorCode.InternalError, `Failed to add entry: ${error}`);
                    }
                }
                case 'get_memory_session': {
                    ...
                    try {
                        ...
                        throw new McpError(ErrorCode.InternalError, `Failed to get session: ${error}`);
                    }
                }
                case 'delete_memory_session': {
                    ...
                    try {
                        ...
                        throw new McpError(ErrorCode.InternalError, `Failed to delete session: ${error}`);
                    }
                }
                default: {
